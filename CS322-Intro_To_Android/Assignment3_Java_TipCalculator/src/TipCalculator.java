...
public class TipCalculator {
	private double billAmount = 0;	//The bill amount: default $0.00
	private int percentageTip = 20; //Percent to tip:  default 20%
	private int partySize = 1;		//Party size:      default 1 person
	...
	public double getIndividualShare() {
		double percent = (double) percentageTip / 100 + 1;	//Convert the integer percentage to a double
		double total = billAmount * percent; //Multiply the bill by the percent for total bill.
		total = (double)Math.round(total / partySize * 100) / 100;	//Divide the total bill by number of people for individual
		return total;
	}
} //End of class
